...
def main():
    ...
    tcra_embeddings, tcrb_embeddings, mhc_embeddings, epitope_embeddings,structural, tcr_ids = [], [], [], [], [], []
    ...
    for start in range(0, num_features, group_size):
        end = min(start + group_size, num_features)
        group_shap = np.abs(shap_values[:, start:end]).mean(axis=1).reshape(-1, 1)  # Reshape to (instances, 1)
        # Almacenar la etiqueta y características estructurales
        grouped_shap_values.append(group_shap)
    
    grouped_shap_values = np.concatenate(grouped_shap_values, axis=1)
    
    print("Grouped SHAP values shape:", grouped_shap_values.shape)
    
    shap.summary_plot(shap_values, X, plot_type="bar")
    plt.savefig(f'shap_summary_plot_{args.type}.png', format='png', dpi=300)
    plt.close()
    
    grouped_shap_means = np.mean(grouped_shap_values, axis=0)  # Average for each group
    grouped_shap_std = np.std(grouped_shap_values, axis=0)


    plt.figure(figsize=(10, 7))
    bars = plt.barh(
        range(grouped_shap_means.shape[0]),
        grouped_shap_means,
        color='cornflowerblue',
        edgecolor='black'
    )

    for bar, value in zip(bars, grouped_shap_means):
        plt.text(bar.get_width(), bar.get_y() + bar.get_height() / 2, f'{value:.2f}', va='center')

    plt.xlabel('Average SHAP Value')
    plt.title(f'Feature Group Importance (Grouped SHAP Values) - {args.type}')
    plt.yticks(range(grouped_shap_means.shape[0]), [f'Group {i+1}' for i in range(grouped_shap_means.shape[0])])

    plt.tight_layout()
    plt.savefig(f'shap_grouped_means_{args.type}.png', format='png', dpi=300)
    plt.close()

    # Versión sin barras de error y con cornflowerblue (modificada)
    plt.figure(figsize=(10, 7))
    plt.barh(
        range(grouped_shap_means.shape[0]),
        grouped_shap_means,
    ...
    plt.tight_layout()
    plt.savefig(f'shap_grouped_summary_{args.type}.png', format='png', dpi=300)
    plt.close()
