...
def process_data_for_type(data_dict, data_type, pooling_function, train=True):
    ...
    return embeddings, labels, tcr_ids

def process_all_data(all_data, type_seq, fold, pooling_function):
    tcra_embeddings_train, tcrb_embeddings_train, epitope_embeddings_train, mhc_embeddings_train, labels_train, tcr_ids_train = [], [], [], [], [], []
    tcra_embeddings_val, tcrb_embeddings_val, epitope_embeddings_val, mhc_embeddings_val, labels_val, tcr_ids_val = [], [], [], [], [], []
    ...
    embeddings_train, labels_train, tcr_ids_train = process_data_for_type(collected_data_train, type_seq, pooling_function)
    ...
    embeddings_val, labels_val, tcr_ids_val = process_data_for_type(collected_data_val, type_seq, pooling_function)
    ...
    return tcra_embeddings_train, tcrb_embeddings_train, epitope_embeddings_train, mhc_embeddings_train, labels_train, tcr_ids_train, \
           tcra_embeddings_val, tcrb_embeddings_val, epitope_embeddings_val, mhc_embeddings_val, labels_val, tcr_ids_val

def create_embedding_matrix(tcra_embeddings, tcrb_embeddings, epitope_embeddings, mhc_embeddings):
...
def main():
    ...
    for learning_rate, max_depth, alpha, reg_lambda in product(
        ...
        for fold in range(1, 6):
            ...
            tcra_embeddings_train, tcrb_embeddings_train, epitope_embeddings_train, mhc_embeddings_train, labels_train, tcr_ids_train, \
            tcra_embeddings_val, tcrb_embeddings_val, epitope_embeddings_val, mhc_embeddings_val, labels_val, tcr_ids_val = process_all_data(all_data, args.type, fold, global_average_pooling)
            ...
            print(f"Length of training data: TCRA {len(tcra_embeddings_train)}, TCRB {len(tcrb_embeddings_train)}, Epitope {len(epitope_embeddings_train)}, MHC {len(mhc_embeddings_train)}, tcrids {len(tcr_ids_train)}, labels {len(labels_train)}")
            print(f"Length of validation data: TCRA {len(tcra_embeddings_val)}, TCRB {len(tcrb_embeddings_val)}, Epitope {len(epitope_embeddings_val)}, MHC {len(mhc_embeddings_val)}, tcrids {len(tcr_ids_val)}, labels {len(labels_val)}")

            #X_train = create_embedding_matrix(tcra_embeddings_train, tcrb_embeddings_train, epitope_embeddings_train, mhc_embeddings_train)
            #X_val = create_embedding_matrix(tcra_embeddings_val, tcrb_embeddings_val, epitope_embeddings_val, mhc_embeddings_val)
            ...
            # Comprobar la consistencia de las dimensiones
            def check_dimensions(X, y, name="data"):
                if X.shape[0] != y.shape[0]:
                    print(f"Warning: Mismatch between {name} and labels size. X: {X.shape[0]}, y: {y.shape[0]}")
                    return False
                return True

            # Crear las matrices de embedding
            X_train = create_embedding_matrix(tcra_embeddings_train, tcrb_embeddings_train, epitope_embeddings_train, mhc_embeddings_train)
            if X_train is not None and check_dimensions(X_train, np.array(labels_train), "train data"):
                print("Training data prepared successfully.")

                
            X_val = create_embedding_matrix(tcra_embeddings_val, tcrb_embeddings_val, epitope_embeddings_val, mhc_embeddings_val)
            if X_val is not None and check_dimensions(X_val, np.array(labels_val), "validation data"):
                print("Validation data prepared successfully.")

            # Convertir TCR IDs y etiquetas a arrays numpy
            tcr_ids_train = np.array(tcr_ids_train)
            tcr_ids_val = np.array(tcr_ids_val)
            ...
            # Verificación de que no haya problemas con los TCR IDs y etiquetas
            if tcr_ids_train.shape[0] != y_train.shape[0]:
                print(f"Warning: Mismatch between tcr_ids_train and y_train size. tcr_ids_train: {tcr_ids_train.shape[0]}, y_train: {y_train.shape[0]}")

            if tcr_ids_val.shape[0] != y_val.shape[0]:
                print(f"Warning: Mismatch between tcr_ids_val and y_val size. tcr_ids_val: {tcr_ids_val.shape[0]}, y_val: {y_val.shape[0]}")
    ...
    tcra_embeddings_train, tcrb_embeddings_train, epitope_embeddings_train, mhc_embeddings_train, labels_train, tcr_ids_train, \
    tcra_embeddings_val, tcrb_embeddings_val, epitope_embeddings_val, mhc_embeddings_val, labels_val, tcr_ids_val = process_all_data(all_data, args.type, 6, global_average_pooling)
    ...
    tcra_embeddings_train, tcrb_embeddings_train, epitope_embeddings_train, mhc_embeddings_train, labels_train, tcr_ids_train, \
    tcra_embeddings_test, tcrb_embeddings_test, epitope_embeddings_test, mhc_embeddings_test, labels_test, tcr_ids_test = process_all_data(all_data, args.type, 7, global_average_pooling)
    X_test = create_embedding_matrix(tcra_embeddings_test, tcrb_embeddings_test, epitope_embeddings_test, mhc_embeddings_test)
